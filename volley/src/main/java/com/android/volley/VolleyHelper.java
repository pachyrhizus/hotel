...
public class VolleyHelper {
		...
		mImageLoader = new ImageLoader(mImageLoaderRequestQueue, new ImageLoader.ImageCache() {
			...
			public Bitmap getBitmap(String url) {
                ...
                if(tmp == null && enabalCahce){
                    //get from local file
                    String localFile = mCtx.getCacheDir().getAbsolutePath() + "/image/" +
                            DiskBasedCache.getFilenameForKey(url);
    ...
    public RequestQueue getImageLoaderRequestQueue() {
        if (mImageLoaderRequestQueue == null) {
            // getApplicationContext() is key, it keeps you from leaking the
            // Activity or BroadcastReceiver if someone passes one in.
            mImageLoaderRequestQueue = Volley.newRequestQueue(mCtx.getApplicationContext(), null, "images", (int)100*1014*1024);
        }
